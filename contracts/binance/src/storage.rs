...
use crate::{Config, DataKey, Error, BinanceData};
...
pub fn set_token_data(env: &Env, token_id: u64, data: &BinanceData) {
...
pub fn get_token_data(env: &Env, token_id: u64) -> Result<BinanceData, Error> {
    let key = (Symbol::new(env, "TOK"), token_id);
    let data: Option<BinanceData> = env.storage().persistent().get(&key);
    if let Some(d) = data {
        env.storage()
            .persistent()
            .extend_ttl(&key, ONE_YEAR, ONE_YEAR);
...
#[allow(dead_code)]
pub fn update_token_data(env: &Env, token_id: u64, data: &BinanceData) -> Result<(), Error> {
    let key = (Symbol::new(env, "TOK"), token_id);
